...
test.describe('Volunteer dashboard', () => {
  test('dashboard renders stadium map and crowd zone cards', async ({ page }) => {
    ...
    await expect(page.locator('.page-title, h2').first()).toBeVisible({ timeout: 10_000 });

    // The stadium map SVG should render
    const stadiumMap = page.locator('svg.zone-map-svg, .stadium-map svg, .stadium-map').first();
    await expect(stadiumMap).toBeVisible({ timeout: 10_000 });

    // Crowd zone cards should render (one per zone)
    const zoneCards = page.locator('.zone-card');
    await expect(zoneCards.first()).toBeVisible({ timeout: 10_000 });
  ...
  test('clicking a zone card expands it to show sparkline', async ({ page }) => {
    ...
    await expect(page.locator('.zone-card--expanded').first()).toBeVisible({ timeout: 5_000 });
  });

  test('dashboard shows live crowd readings with density bars', async ({ page }) => {
    await page.goto('/dashboard');

    // Density progress bars should render
    const densityBars = page.locator('.zone-density-bar, [role="progressbar"]');
    await expect(densityBars.first()).toBeVisible({ timeout: 10_000 });
